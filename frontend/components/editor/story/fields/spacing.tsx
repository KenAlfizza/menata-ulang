...
const parseSpacing = (val: string | undefined): { num: string; unit: Unit } => {
...
export const spacingField: SpacingCustomField = {
    ...
    render: ({ value, onChange }) => {
        const current = value || defaultSpacing;

        const handleChange = (side: keyof Spacing, num: string, unit: Unit) => {
        ...
        const renderInput = (side: keyof Spacing) => {
            const { num, unit } = parseSpacing(current[side]);
            return (
                    ...
                    <div className="grid w-full grid-cols-2 gap-2">
                        {[
                            ...
                            <div key={side} className="flex items-center justify-between gap-2 rounded border border-slate-200 bg-white p-1.5">
                                ...
                                {renderInput(side as keyof Spacing)}
                            </div>
                        ))}
                    </div>
                </div>
